...
class BlogBuilder:
    ...
    def build(self):
        ...
        # Load templates once
        try:
            post_template = self.env.get_template("base_new.html")
            index_template = self.env.get_template("index_new.html")
            error_template = self.env.get_template("404_new.html")
        except Exception as e:
            # Fallback to old templates if new ones don't exist
            try:
